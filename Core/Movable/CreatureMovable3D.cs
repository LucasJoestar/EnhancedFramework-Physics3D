...
namespace EnhancedFramework.Physics3D {
    ...
    public sealed class CreatureMovable3D : Movable3D {
        ...
        protected override bool OnPreComputeVelocity(float _deltaTime) {
            if (base.OnPreComputeVelocity(_deltaTime)) {
                return true;
            }
        ...
        protected override bool OnPostComputeVelocity(ref FrameVelocity _velocity, float _deltaTime) {
            if (base.OnPostComputeVelocity(ref _velocity, _deltaTime)) {
                return true;
            }
        ...
        protected override float ComputeSpeed(float _speed, float _deltaTime) {
            _speed = base.ComputeSpeed(_speed, _deltaTime);
        ...
        private void ComputeMovementVelocity(float _deltaTime) {
            ...
            if (_deceleration || _turnAround) {
                ...
                // Computes movement when direction changed.
                if (isChangingDirection || (!_lastMovement.IsNull() && (Vector3.Dot(_lastMovement - _movement, _lastMovement) >= 0f))) {
                    bool _isChangingDirection;

                    // Turn around.
                    if (_turnAround && (_movement.x != 0f) && (Vector3.Dot(_lastMovement, _movement) <= .1f)) {

                        float _speed = attributes.TurnAroundSpeed * PhysicsSurface.ChangeDirectionCoef;
                        _movement = Vector3.MoveTowards(_lastMovement, _movement, _deltaTime * _speed);

                        _isChangingDirection = true;
                    }
                    // Deceleration.
                    else if (_deceleration && (_lastMovement != _movement)) {
                    ...
                    isChangingDirection = _isChangingDirection;
                }
