...
namespace EnhancedFramework.Physics3D {
    ...
    internal abstract class PhysicsSystem3D<T> where T : PhysicsSystem3D<T>, new() {
        ...
        public void CollisionFinalizeOperation  (Movable3D _movable, CollisionOperationData3D _operation) {
            ...
            // Compute final state.
            _operation.Data.AppliedVelocity = _movable.Position - _operation.OriginPosition;
        ...
        protected virtual bool OnComputeGround(Movable3D _movable, CollisionOperationData3D _operation) {
            ...
            if (!_isGrounded && _movable.UseGravity) {
                ...
                if (!_isGrounded) {
                    ...
                    float _distance = _collider.ContactOffset * 2.5f;

                    CollisionHit3D _movableHit = new CollisionHit3D();

                    if ((_collider.Raycast(-_movable.GroundNormal, out RaycastHit _hit, _distance)                           && IsGroundSurface(_movable, _hit)) ||
                        (PerformCast(_movable, _operation, _movable.GravitySense * _distance, out _movableHit, false, false) && IsGroundSurface(_movable, _movableHit.RaycastHit))) {

                        // Compute and register impact.
                        if (_movableHit.RaycastHit.colliderInstanceID == 0) {
                            _movableHit = new CollisionHit3D(_hit, _movable.Collider, false);
                        }

                        // If found, set ground.
                        _isGrounded = true;
                        _groundHit  = _movableHit.RaycastHit;

                        ComputeImpact(_movable, _operation, _movableHit);
                    }
                }
            }
