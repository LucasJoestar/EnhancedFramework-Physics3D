...
namespace EnhancedFramework.Physics3D {
    ...
    public abstract class Movable3D : EnhancedBehaviour, IMovable3D, ITriggerActor {
        ...
        [SerializeField, Enhanced, ReadOnly] private PhysicsSurface3D.Settings physicsSurface = PhysicsSurface3D.Settings.Default;
        ...
        [SerializeField] private Vector2 transferVelocityRange = new Vector2(0f, 0f);
        ...
        [SerializeField, Enhanced, EnhancedCurve(0f, 0f, 1f, 1f)] private AnimationCurve transferVelocityCurve = AnimationCurve.Linear(0f, 1f, 1f, 1f);

        [Space(10f)]

        [Tooltip("Coefficient combined with the above curve-range parameters to transfer this object velocity")]
        [SerializeField, Enhanced, Range(0f, 1f)] private float transferVelocityApply = .6f;

        [Tooltip("Coefficient combined with the above curve-range parameters to dampen this object own velocity during transfer")]
        [SerializeField, Enhanced, Range(0f, 1f)] private float transferVelocitySelfReduce = .5f;
        ...
        internal bool LogicPrepareCollisions(float _deltaTime, Physics3DSettings _settings, out CollisionOperationData3D _collisionOperation) {
            ...
            OnPreComputeVelocity(_deltaTime);
            ComputeVelocity(_settings, _deltaTime, out FrameVelocity _velocity);

            OnPostComputeVelocity(ref _velocity, _deltaTime);
            ...
            _collisionOperation.Init(_settings, this, _position, _velocity, _performCollisions, _performCollisions ? selfColliders : null);
        ...
        internal bool LogicOnCollisionComplete(CollisionOperationData3D _operation, out ExtractOperation3D _extractOperation) {
            ...
            if (!_operation.Data.AppliedVelocity.IsNull()) {
        ...
        internal void LogicLateUpdate(CollisionOperationData3D _operation) {
            ...
            _operation.Data.AppliedVelocity = Position - _operation.OriginPosition;
        ...
        /// </summary>
        /// <returns><inheritdoc cref="Doc" path="/returns"/></returns>
        protected virtual bool OnPreComputeVelocity(float _deltaTime) {
        ...
        /// <returns><inheritdoc cref="Doc" path="/returns"/></returns>
        protected virtual bool ComputeVelocity(Physics3DSettings _settings, float _deltaTime, out FrameVelocity _frameVelocity) {

            Velocity _velocity = this.Velocity;
            _frameVelocity = new FrameVelocity();

            if (computationController.OnComputeVelocity(_velocity, ref _frameVelocity)) {
                return true;
            }
            ...
            Vector3 _movement    = GetRelativeVector(_velocity.Movement, _rotation);
            Vector3 _force       = GetRelativeVector(_velocity.Force, _rotation);

            float _speed = ComputeSpeed(speed, _deltaTime);

            // Add instant movement.
            if ((_deltaTime != 0f) && (_speed != 0f)) {
                Vector3 _instantMovement = GetRelativeVector(_velocity.InstantMovement, _rotation);
            ...
            _frameVelocity = new FrameVelocity() {
                Movement   = GetWorldVector(GetAxisVelocity(_movement, false), _rotation) * _veloctyDelta,
                Force      = GetWorldVector(GetAxisVelocity(_force,    false), _rotation) * _veloctyDelta,
            ...
            // Update velocity.
            _velocity.Force = GetWorldVector(_force, _rotation);
        ...
        /// <returns><inheritdoc cref="Doc" path="/returns"/></returns>
        protected virtual bool OnPostComputeVelocity(ref FrameVelocity _velocity, float _deltaTime) {
        ...
        /// <returns></returns>
        protected virtual float ComputeSpeed(float _speed, float _deltaTime) {
            return _speed;
        }
        ...
        protected virtual bool OnRefreshedObject(CollisionOperationData3D _operation) {
            return collisionController.OnRefreshedObject(_operation);
        }
        ...
        internal protected virtual bool OnExtractFromCollider(Collider _collider, Vector3 _direction, float _distance) {
            ...
            if (_collider.TryGetComponentInParent(out Movable3D _other) && _other.enabled) {
            ...
            if (debugCollisions) {
                this.LogMessage("Extract from Collider => " + _collider.name + " - " + _offset.ToStringX(3) + " - " + RockBehaviour);
            }
            #endif
        ...
        internal protected virtual bool SetGroundState(CollisionOperationData3D _operation, bool _isGrounded, RaycastHit _hit) {
            ...
            if (isGrounded) {
                ...
                groundNormal =  _normal;
                gravitySense = -_normal;
            }
        ...
        internal bool ExtractFromCollider(Collider _colliderA, Collider _colliderB, bool _ignoreSelfColliders = true) {
            ...
            if (Physics.ComputePenetration(_colliderA, _aTransform.position, _aTransform.rotation,
                ...
                // Collider extraction.
                OnExtractFromCollider(_colliderB, _direction, _distance);
            }
        ...
        public bool CanPushObject {
            get { return (pushRange.y >= MinEffectWeight); }
        }
        ...
        /// </summary>
        public bool CanTransferVelocity {
            get { return transferVelocityRange.y >= MinEffectWeight; }
        }
        ...
        public float GetPushVelocityCoef(Movable3D _other) {
            ...
            return GetCollisionEffectValue(_other.weight, pushRange, pushCurve);
        }
        ...
        public bool GetTransferVelocityValues(Movable3D _other, out float _reduceSelfCoef, out float _applyCoef) {
            ...
            float _value = GetCollisionEffectValue(_other.weight, transferVelocityRange, transferVelocityCurve);

            _reduceSelfCoef = transferVelocitySelfReduce * _value;
            _applyCoef      = transferVelocityApply      * _value;
